...
export class ImportExportService {
    ...
    try {
      ...
      switch (options.format) {
        ...
        case 'json':
          ...
          break;
        case 'pbm':
          importData = await this.parsePbmFile(options.fileContent, options.decryptionKey);
          break;
        default:
      ...
      // 3. 执行导入策略
      const result = await this.executeImportStrategy(importData, options.strategy);
   ...
   * 覆盖导入
   */
  private async overwriteImport(importData: ImportExportData): Promise<ImportResult> {
  ...
  async exportBookmarks(options: ExportOptions): Promise<ExportResult> {
    try {
      ...
      switch (options.format) {
        case 'json-encrypted':
          return await this.exportJsonEncrypted(bookmarks, folders, tags, options.encryptionKey);
        case 'json-plain':
   ...
   * 导出 HTML
   */
  private async exportHtml(bookmarks: Bookmark[], folders: Folder[]): Promise<ExportResult> {
  ...
  private async writeFolders(folders: Folder[]): Promise<void> {
    try {
      ...
      const existingFolders = existingResult.success && existingResult.data ? existingResult.data : [];
  ...
  private async writeTags(tags: Tag[]): Promise<void> {
    try {
      ...
      const existingTags = existingResult.success && existingResult.data ? existingResult.data : [];
      ...
      for (const tag of tags) {
        ...
        if (existingIds.has(tag.id) || existingNames.has(tag.name.toLowerCase())) {
          continue;
        }
