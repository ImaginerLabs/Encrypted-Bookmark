...
const ImportSection: React.FC<ImportSectionProps> = ({
  ...
  const validateFileFormat = useCallback(
    (file: File, format: ImportFormat): boolean => {
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (!validateFileFormat(file, importFormat)) {
          throw new Error(
            `文件格式不匹配，期望 ${getFormatExtension(importFormat)} 格式`,
          );
        }
        ...
        const successMessage = [
          ...
          result.skippedBookmarks
            ? `跳过: ${result.skippedBookmarks} 个（重复）`
            : "",
          result.deletedBookmarks
            ? `覆盖: ${result.deletedBookmarks} 个旧书签`
            : "",
        ]
          .filter(Boolean)
          .join("，");
        ...
        onMessage(error instanceof Error ? error.message : "导入失败", "error");
      } finally {
        setImporting(false);
        setTimeout(() => {
          setProgress(0);
  ...
  const handleFileSelect = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file.size > MAX_FILE_SIZE) {
        onMessage(
          `文件过大，最大支持 50MB（当前文件：${(file.size / 1024 / 1024).toFixed(2)}MB）`,
          "error",
        );
        if (fileInputRef.current) {
          fileInputRef.current.value = "";
    ...
    <div className="settings-section">
      ...
      <div className="form-group">
        ...
        <select
          ...
          disabled={importing}
        >
          {(Object.keys(FORMAT_CONFIG) as ImportFormat[]).map((key) => (
            <option key={key} value={key}>
              {FORMAT_CONFIG[key].label}
            </option>
          ))}
        </select>
        <small className="form-hint">{FORMAT_CONFIG[importFormat].hint}</small>
      </div>
