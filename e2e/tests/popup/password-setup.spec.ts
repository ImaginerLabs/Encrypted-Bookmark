...
test.describe("Popup - 密码设置", () => {
  ...
  test("首次使用显示设置密码界面", async ({ popupPage }) => {
    ...
    // 验证密码输入框存在
    await expect(
      popupPage.locator('input[placeholder="8-32 位字符"]'),
    ).toBeVisible();
    await expect(
      popupPage.locator('input[placeholder="再次输入密码"]'),
    ).toBeVisible();

    // 验证设置按钮存在
    await expect(
      popupPage.locator('button:has-text("设置密码")'),
    ).toBeVisible();

    // 验证提示信息
    await expect(popupPage.locator(PopupSelectors.tips)).toContainText(
      "请牢记此密码",
    );
  });
  ...
  test("密码过长时显示错误", async ({ popupPage }) => {
    ...
    // 验证错误提示
    await expect(popupPage.locator(PopupSelectors.error)).toBeVisible();
    await expect(popupPage.locator(PopupSelectors.error)).toContainText("过长");
  });
