...
test.describe('Popup - 书签管理界面', () => {
  ...
  async function setupUnlocked(popupPage: import('@playwright/test').Page) {
    ...
    await popupPage.fill('input[placeholder="8-32 位字符"]', 'BookmarkTest123!');
    await popupPage.fill('input[placeholder="再次输入密码"]', 'BookmarkTest123!');
  ...
  test('显示搜索框', async ({ extensionContext, extensionId }) => {
    ...
    await expect(popupPage.locator('.search-input')).toHaveAttribute('placeholder', '搜索书签...');
  ...
  test('无书签时显示空状态', async ({ extensionContext, extensionId }) => {
    ...
    // 验证空状态（没有书签数据）
    await expect(popupPage.locator('.popup-body')).toContainText('暂无书签', { timeout: 5000 });
  ...
  test('底部显示锁定按钮', async ({ extensionContext, extensionId }) => {
    ...
    await expect(popupPage.locator('.btn-lock')).toBeVisible();
    await expect(popupPage.locator('.btn-lock')).toContainText('锁定');
  ...
  test('点击锁定按钮重新加载页面', async ({ extensionContext, extensionId }) => {
    const popupPage = await createPopupPage(extensionContext, extensionId);
    await setupUnlocked(popupPage);

    await popupPage.click('.btn-lock');
    await popupPage.waitForLoadState('domcontentloaded');
    await expect(popupPage.locator('.loading')).not.toBeVisible({ timeout: 10000 });
