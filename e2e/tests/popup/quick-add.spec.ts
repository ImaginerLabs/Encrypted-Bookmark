...
test.describe("Popup - 快速添加书签", () => {
  ...
  test("点击添加按钮打开快速添加面板", async ({
    ...
    await expect(popupPage.locator(".quick-add-panel h3")).toContainText(
      "添加书签",
    );
  ...
  test("面板显示标题和URL输入框", async ({ extensionContext, extensionId }) => {
    ...
    // 验证表单字段
    await expect(popupPage.locator("#qa-title")).toBeVisible();
    await expect(popupPage.locator("#qa-url")).toBeVisible();
    await expect(popupPage.locator("#qa-folder")).toBeVisible();
    await expect(popupPage.locator("#qa-tags")).toBeVisible();
    await expect(popupPage.locator("#qa-note")).toBeVisible();
  ...
  test("面板显示取消和保存按钮", async ({ extensionContext, extensionId }) => {
    ...
    await expect(popupPage.locator(".quick-add-btn-cancel")).toBeVisible();
    await expect(popupPage.locator(".quick-add-btn-cancel")).toContainText(
      "取消",
    );
    await expect(popupPage.locator(".quick-add-btn-save")).toBeVisible();
    await expect(popupPage.locator(".quick-add-btn-save")).toContainText(
      "保存",
    );
  ...
  test("空标题提交显示验证错误", async ({ extensionContext, extensionId }) => {
    ...
    await popupPage.locator("#qa-title").clear();
    await popupPage.fill("#qa-url", "https://example.com");

    // 确保标题确实为空
    await expect(popupPage.locator("#qa-title")).toHaveValue("");

    // 等待面板可见后点击保存
    await expect(popupPage.locator(".quick-add-btn-save")).toBeEnabled({
      timeout: 3000,
    });
    await popupPage.click(".quick-add-btn-save");

    // 验证错误提示（在 quick-add-error 或 quick-add-general-error 中）
    const errorLocator = popupPage.locator(
      ".quick-add-error, .quick-add-general-error",
    );
    await expect(errorLocator.first()).toBeVisible({
      timeout: 5000,
    });
    await expect(errorLocator.first()).toContainText("标题");
  ...
  test("无效URL提交显示验证错误", async ({ extensionContext, extensionId }) => {
    ...
    await popupPage.fill("#qa-title", "测试书签");
    await popupPage.fill("#qa-url", "not-a-valid-url");
    await popupPage.click(".quick-add-btn-save");

    await expect(popupPage.locator(".quick-add-error")).toBeVisible({
      timeout: 5000,
    });
    await expect(popupPage.locator(".quick-add-error")).toContainText(
      "有效的 URL",
    );
  ...
  test("点击取消关闭面板", async ({ extensionContext, extensionId }) => {
    ...
    // 填写一些数据
    await popupPage.fill("#qa-title", "临时书签");
  ...
  test("点击遮罩层关闭面板", async ({ extensionContext, extensionId }) => {
    ...
    await popupPage.evaluate(() => {
      (document.querySelector(".quick-add-overlay") as HTMLElement)?.click();
    });
    await expect(popupPage.locator(".quick-add-panel")).not.toBeVisible();
  ...
  test("文件夹选择器默认显示未分类", async ({
    ...
    // 验证默认选项
    const folderSelect = popupPage.locator("#qa-folder");
