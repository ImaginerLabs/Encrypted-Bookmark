...
test.describe('Popup - 账户锁定机制', () => {
  test('账户锁定时显示锁定界面', async ({ popupPage }) => {
    ...
    // 验证显示锁定标题
    await expect(popupPage.locator('.header h1')).toContainText('账户已锁定');

    // 验证显示锁定信息
    await expect(popupPage.locator('.locked-info')).toBeVisible();
  });
  ...
  test('锁定界面显示剩余等待时间', async ({ popupPage }) => {
    ...
    // 验证显示剩余时间
    await expect(popupPage.locator('.locked-info')).toContainText('秒后重试');
  });
  ...
  test('锁定界面显示失败次数', async ({ popupPage }) => {
    ...
    // 验证显示失败次数
    await expect(popupPage.locator('.attempts')).toContainText('失败次数：5');
  });
  ...
  test('锁定界面不显示密码输入框', async ({ popupPage }) => {
    ...
    // 验证没有密码输入框
    await expect(popupPage.locator('input[placeholder="输入密码"]')).not.toBeVisible();
  });
  ...
  test('锁定界面不显示解锁按钮', async ({ popupPage }) => {
    ...
    // 验证没有解锁按钮
    await expect(popupPage.locator('button:has-text("解锁")')).not.toBeVisible();
  });
...
test.describe('Popup - 锁定阈值', () => {
  test('2 次错误后不锁定', async ({ popupPage }) => {
    ...
    // 应该仍在解锁界面
    await expect(popupPage.locator('input[placeholder="输入密码"]')).toBeVisible();
    await expect(popupPage.locator('.warning')).toContainText('已失败 2 次');
  });
  ...
  test('3 次错误后锁定 30 秒', async ({ extensionContext, extensionId }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (i < 2) {
        // 等待错误提示
        await expect(popupPage.locator('.error')).toBeVisible({ timeout: 10000 });
        // 等待一下再继续
