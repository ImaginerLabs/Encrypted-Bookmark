...
test.describe('Popup - 解锁流程', () => {
  ...
  test('输入错误密码显示错误提示', async ({ popupPage }) => {
    ...
    await expect(popupPage.locator('.error')).toContainText('密码错误');
  });
  ...
  test('错误密码后显示剩余尝试次数', async ({ popupPage }) => {
    ...
    // 验证显示剩余次数
    await expect(popupPage.locator('.error')).toContainText('剩余尝试次数', {
      timeout: 10000,
    });
  });
  ...
  test('显示失败次数警告', async ({ popupPage }) => {
    ...
    // 验证显示失败次数警告
    await expect(popupPage.locator('.warning')).toBeVisible();
  ...
  test('失败 3 次后显示锁定风险警告', async ({ popupPage }) => {
    ...
    // 验证显示锁定风险警告
    await expect(popupPage.locator('.warning')).toContainText('锁定风险');
  });
...
test.describe('Popup - 解锁成功', () => {
  test('通过 UI 设置密码后能成功解锁', async ({ extensionContext, extensionId }) => {
