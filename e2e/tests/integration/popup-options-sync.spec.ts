...
test.describe('Popup-Options 状态同步', () => {
  test('Popup 设置密码后 Options 反映变化', async ({ extensionContext, extensionId, optionsUrl }) => {
    ...
    // 验证 Options 能感知密码已设置
    // 切换到安全设置查看状态
    const securityTab = optionsPage.locator('[data-tab="security"], .nav-item:has-text("安全")');
  ...
  test('多个 Popup 实例状态同步', async ({ extensionContext, extensionId }) => {
    ...
    await expect(popup1.locator('.loading')).not.toBeVisible({ timeout: 10000 });
    ...
    // 第二个 Popup 应该显示解锁界面（密码已设置）
    // 注意：解锁状态是在内存中，所以新 Popup 应该需要解锁
  ...
  test('Popup 锁定后其他页面同步', async ({ extensionContext, extensionId }) => {
    const popup1 = await createPopupPage(extensionContext, extensionId);
    const storage = createStorageHelper(popup1);
    await storage.clear();
    await popup1.reload();
    ...
    // 在第一个 Popup 锁定
    await popup1.click('button:has-text("锁定")');
    await popup1.waitForLoadState('domcontentloaded');
    await expect(popup1.locator('.loading')).not.toBeVisible({ timeout: 10000 });
  ...
  test('Storage 变化触发 UI 更新', async ({ extensionContext, extensionId }) => {
    ...
    await expect(popup.locator('input[placeholder="8-32 位字符"]')).toBeVisible();
 ...
 */
test.describe('跨页面导航', () => {
  test('从 Popup 打开 Options 页面', async ({ extensionContext, extensionId, optionsUrl }) => {
    const popup = await createPopupPage(extensionContext, extensionId);
