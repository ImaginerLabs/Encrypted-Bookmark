...
test.describe("Options - 导入导出面板解锁", () => {
  /** 点击导入导出 Tab */
  async function clickImportExportTab(
  ...
  test("未登录时显示提示和解锁表单", async ({
    ...
    await expect(
      optionsPage.locator('button.btn-primary:has-text("解锁")'),
    ).toBeVisible();

    await optionsPage.close();
  });

  test("密码已设置但未解锁时可通过表单解锁", async ({
    extensionContext,
    extensionId,
    optionsUrl,
  }) => {
    ...
    await expect(
      optionsPage.locator('input[placeholder="输入密码"]').first(),
    ).not.toBeVisible();
    // 应显示导入相关内容
    await expect(optionsPage.locator(".panel-content")).toContainText("导入", {
      timeout: 10000,
    });
