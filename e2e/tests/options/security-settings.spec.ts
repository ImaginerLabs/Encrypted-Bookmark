...
test.describe("Options - 安全设置", () => {
  test("安全设置面板可访问", async ({ optionsPage }) => {
    ...
    await expect(optionsPage.locator(".options-content")).toContainText(
      /安全|密码/,
    );
  });
  ...
  test("显示修改密码选项", async ({ optionsPage }) => {
    await optionsPage.locator(OptionsSelectors.tabSecurity).click();
    // 等待安全设置面板加载完成
    await expect(optionsPage.locator(".panel-title")).toContainText(
      "安全设置",
      { timeout: 5000 },
    );
    const content = optionsPage.locator(".options-content");
    await expect(content.locator("text=修改主密码")).toBeVisible({
      timeout: 5000,
    });
  });
});
