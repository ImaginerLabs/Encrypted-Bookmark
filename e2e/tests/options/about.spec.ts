...
test.describe("Options - 关于面板", () => {
  ...
  test("显示应用名称和版本", async ({ optionsPage }) => {
    ...
    await expect(optionsPage.locator(".app-name")).toContainText(
      "Encrypted Bookmark",
    );
    await expect(optionsPage.locator(".app-version")).toContainText("版本");
    await expect(optionsPage.locator(".app-version")).toContainText("1.1.0");
  });
  ...
  test("显示技术栈信息", async ({ optionsPage }) => {
    ...
    await expect(optionsPage.locator(".tech-list")).toContainText(
      "Manifest V3",
    );
  });
  ...
  test("显示开源许可信息", async ({ optionsPage }) => {
    ...
    await expect(
      optionsPage.locator('.info-section:has-text("开源")'),
    ).toContainText("MIT 许可");
  });
  ...
  test("显示隐私政策", async ({ optionsPage }) => {
    ...
    await expect(optionsPage.locator(".privacy-list")).toContainText(
      "不收集任何用户数据",
    );
  ...
  test("显示危险操作区域和重置按钮", async ({ optionsPage }) => {
    ...
    await expect(optionsPage.locator(".btn-danger")).toContainText(
      "重置所有数据",
    );
  });
});
